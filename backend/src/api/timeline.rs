...
async fn list_record_activities(
    ...
    Ok(Json(state.store.query_activities(&query, limit, offset).await?))
}
...
async fn create_record_activity(
    State(state): State<AppState>,
    Path(record_id): Path<String>,
...
struct CreateTaskBody {
    /// Absent for a standalone task — one with no record attached, which is the case
    /// the record-scoped route cannot express.
    #[serde(default)]
    record_id: Option<String>,
    #[serde(default)]
    title: String,
    ...
    #[serde(default)]
    assignee: Option<String>,
    /// Anything `models::normalize_datetime` accepts, including a bare `YYYY-MM-DD`.
    #[serde(default)]
