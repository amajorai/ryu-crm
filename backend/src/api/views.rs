...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/lists/:list_id/entries/reorder",
            post(reorder_list_entries),
        )
...
#[utoipa::path(
    get,
    path = "/api/crm/objects/{object}/views",
    tag = "CRM",
    summary = "list one object's saved views — the tables, boards and lists someone has already set up for it.",
...
#[utoipa::path(
    ...
    path = "/api/crm/objects/{object}/views",
    tag = "CRM",
    summary = "save a new view on an object — a named table, board or list with its own filter, sorts and visible columns.",
    params(("object" = String, Path, description = "Object id or slug.")),
...
#[utoipa::path(
    get,
    path = "/api/crm/views/{view_id}",
    tag = "CRM",
    summary = "read one saved view's definition.",
    params(("view_id" = String, Path, description = "The view's id.")),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
...
#[utoipa::path(
    ...
    path = "/api/crm/views/{view_id}",
    tag = "CRM",
    summary = "change a saved view's name, filter, sorts, columns or grouping.",
    params(("view_id" = String, Path, description = "The view's id.")),
    request_body = UpdateViewRequest,
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
pub(crate) async fn patch_view(
    State(state): State<AppState>,
    Path(view_id): Path<String>,
...
#[utoipa::path(
    post,
    path = "/api/crm/views/{view_id}/default",
    tag = "CRM",
    summary = "make this the view that opens when someone opens its object.",
    params(("view_id" = String, Path, description = "The view's id.")),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
pub(crate) async fn set_default_view(
    State(state): State<AppState>,
    Path(view_id): Path<String>,
) -> ApiResult<Json<View>> {
...
// NOT `Option<ViewQueryOverrides>`, even though the handler accepts an absent body.
// utoipa renders an optional request body as `{"oneOf":[{"type":"null"},{"$ref":…}]}`,
// which buries the real argument shape one level down where the derived tool cannot
// show it. The plain type is the honest description of what a caller may send.
#[utoipa::path(
    post,
    path = "/api/crm/views/{view_id}/run",
    tag = "CRM",
    summary = "run a saved view and get the rows back — the table page or the board's columns, already filtered and sorted the way the view says.",
    params(("view_id" = String, Path, description = "The view's id.")),
    request_body = ViewQueryOverrides,
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
...
pub(crate) struct ListScopeQuery {
    #[serde(default)]
...
#[utoipa::path(
    ...
    path = "/api/crm/lists",
    tag = "CRM",
    summary = "list the CRM's curated lists — hand-picked collections of records with their own extra columns.",
    params(("object_id" = Option<String>, Query, description = "Restrict to one object's lists. Omit for every list on the node.")),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
...
#[utoipa::path(
    ...
    path = "/api/crm/lists",
    tag = "CRM",
    summary = "create a curated list on one object.",
...
#[utoipa::path(
    get,
    path = "/api/crm/lists/{list_id}",
    tag = "CRM",
    summary = "read one curated list's definition.",
    params(("list_id" = String, Path, description = "The list's id.")),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
...
#[utoipa::path(
    ...
    path = "/api/crm/lists/{list_id}",
    tag = "CRM",
    summary = "rename a curated list or change its icon, description or position.",
    params(("list_id" = String, Path, description = "The list's id.")),
...
#[utoipa::path(
    ...
    path = "/api/crm/lists/{list_id}",
    tag = "CRM",
    summary = "delete a curated list and its entries. The records themselves are untouched.",
    params(("list_id" = String, Path, description = "The list's id.")),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
...
#[utoipa::path(
    get,
    path = "/api/crm/lists/{list_id}/fields",
    tag = "CRM",
    summary = "list a curated list's OWN extra fields — the columns that exist only inside this list.",
    params(("list_id" = String, Path, description = "The list's id.")),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
pub(crate) async fn list_list_fields(
    State(state): State<AppState>,
    Path(list_id): Path<String>,
) -> ApiResult<Json<Value>> {
...
#[utoipa::path(
    ...
    path = "/api/crm/lists/{list_id}/fields",
    tag = "CRM",
    summary = "add an extra field that exists only inside one curated list.",
    params(("list_id" = String, Path, description = "The list's id.")),
    request_body = CreateFieldRequest,
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
pub(crate) async fn create_list_field(
    State(state): State<AppState>,
    Path(list_id): Path<String>,
...
#[utoipa::path(
    post,
    path = "/api/crm/lists/{list_id}/entries",
    tag = "CRM",
    summary = "put a record onto a curated list, optionally with values for the list's own extra fields.",
    params(("list_id" = String, Path, description = "The list's id.")),
    request_body = AddListEntryRequest,
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
pub(crate) async fn add_list_entry(
    State(state): State<AppState>,
    Path(list_id): Path<String>,
...
// Plain type, not `Option<…>` — see the note on `run_view`.
#[utoipa::path(
    post,
    path = "/api/crm/lists/{list_id}/entries/query",
    tag = "CRM",
    summary = "read a curated list's entries with their records resolved, filtered and sorted over both the record's fields and the list's own.",
    params(("list_id" = String, Path, description = "The list's id.")),
    request_body = ListEntryQuery,
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
pub(crate) async fn query_list_entries(
    State(state): State<AppState>,
    Path(list_id): Path<String>,
    ...
    Ok(Json(
        state
            .store
            .query_list_entries(&query, limit, offset)
            .await?,
    ))
}
...
#[utoipa::path(
    post,
    path = "/api/crm/lists/{list_id}/entries/reorder",
    tag = "CRM",
    summary = "set the manual order of a curated list's entries.",
    params(("list_id" = String, Path, description = "The list's id.")),
    request_body = ReorderRequest,
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
pub(crate) async fn reorder_list_entries(
    State(state): State<AppState>,
    Path(list_id): Path<String>,
    ...
    state
        .store
        .reorder_list_entries(&list.id, &body.ids)
        .await?;
    Ok(Json(json!({ "ok": true })))
}
...
#[utoipa::path(
    patch,
    path = "/api/crm/list-entries/{entry_id}",
    tag = "CRM",
    summary = "update one list entry's list-specific values.",
    params(("entry_id" = String, Path, description = "The entry's id — not the record's.")),
    request_body = UpdateListEntryRequest,
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
pub(crate) async fn patch_list_entry(
    State(state): State<AppState>,
...
#[utoipa::path(
    ...
    path = "/api/crm/list-entries/{entry_id}",
    tag = "CRM",
    summary = "take a record off a curated list. The record itself is untouched.",
    params(("entry_id" = String, Path, description = "The entry's id — not the record's.")),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
pub(crate) async fn remove_list_entry(
    State(state): State<AppState>,
    Path(entry_id): Path<String>,
) -> ApiResult<Json<Value>> {
    require_hit(
        state.store.remove_list_entry(&entry_id).await?,
        "list entry",
    )?;
    Ok(Json(json!({ "ok": true })))
}
...
mod tests {
    ...
    async fn the_path_list_id_overrides_the_body() {
        ...
        assert_eq!(
            page.total, 0,
            "the body must not redirect the read to list A"
        );
