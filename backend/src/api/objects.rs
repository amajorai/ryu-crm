...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/objects/:object/fields", get(list_fields).post(create_field))
...
async fn schema(State(state): State<AppState>) -> ApiResult<Json<SchemaResponse>> {
    Ok(Json(state.store.schema().await?))
}

// ── Objects ────────────────────────────────────────────────────────────────────

async fn list_objects(State(state): State<AppState>) -> ApiResult<Json<Vec<ObjectSummary>>> {
...
async fn plan_option_migration(
    ...
    plan.rewrite_first = plan.remap.values().all(|target| match target {
        ...
        Some(raw) => incoming
            .resolve_option(raw)
            .is_some_and(|option| !option.id.is_empty() && field.config.option(&option.id).is_some()),
    });
...
mod tests {
    ...
    fn a_scalar_option_is_replaced_or_cleared() {
        ...
        assert_eq!(migrate_option_value(Some(&json!("opt_c")), &plan), Value::Null);
    }
    ...
    async fn reorder_refuses_ids_from_another_object_and_duplicates() {
        ...
        assert!(matches!(duplicated, ApiError::BadRequest(_)), "{duplicated}");
        ...
        .0;
        assert_eq!(ordered.first().map(|f| f.id.as_str()), Some("fld_company_domain"));
    }
    ...
    async fn dropping_an_in_use_option_is_refused_with_the_option_named() {
        ...
        assert!(message.contains("Lead"), "the option must be named: {message}");
        assert!(message.contains('2'), "the blast radius must be stated: {message}");
    ...
    async fn a_migration_onto_a_brand_new_option_resolves_after_the_store_assigns_its_id() {
        ...
        config
            .options
            .push(SelectOption::new("", "Evaluating", config.options.len() as i64));
