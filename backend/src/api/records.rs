...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/records/:record_id/links",
            get(list_links).post(link_records),
        )
...
#[utoipa::path(
    get,
    path = "/api/crm/objects/{object}/records",
    tag = "CRM",
    summary = "list one object's records — a plain page of companies, people, deals, or any custom object.",
    params(
        ("object" = String, Path, description = "Object id or slug, e.g. `company`, `person`, `deal`."),
        ("search" = Option<String>, Query, description = "Full-text pre-filter over the object's searchable fields."),
        ("sort" = Option<String>, Query, description = "Field id, field slug, or one of `title` / `created_at` / `updated_at`. Defaults to most-recently-updated first."),
        ("direction" = Option<String>, Query, description = "`asc` or `desc`."),
        ("limit" = Option<usize>, Query, description = "Page size. Clamped to the node's ceiling."),
        ("offset" = Option<usize>, Query, description = "How many records to skip."),
        ("include_deleted" = Option<bool>, Query, description = "Include soft-deleted records. Defaults to false."),
        ("list_id" = Option<String>, Query, description = "Restrict to the members of one curated list.")
    ),
...
pub(crate) async fn list_records(
    ...
    Ok(Json(
        state.store.query_records(&query, limit, offset).await?,
    ))
}
...
// The `object_id` field of the body is documented but IGNORED — the path wins. Left
// in the schema rather than hidden, because `RecordQuery` is also the stored shape of
// a saved view and a caller round-tripping one would be surprised to find the field
// gone.
#[utoipa::path(
    post,
    path = "/api/crm/objects/{object}/records/query",
    tag = "CRM",
    summary = "query one object's records with a filter tree, sorts, and an optional full-text pre-filter — the precise counterpart to a plain list.",
    params(("object" = String, Path, description = "Object id or slug. Wins over any `object_id` in the body.")),
    request_body = RecordQuery,
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
...
#[utoipa::path(
    get,
    path = "/api/crm/records/{record_id}",
    tag = "CRM",
    summary = "read one record by id, optionally with its whole drawer — schema, links, recent timeline and list memberships.",
    params(
        ("record_id" = String, Path, description = "The record's id."),
        ("detail" = Option<bool>, Query, description = "True returns the record plus its object, fields, links, newest activities and list memberships in one round trip.")
    ),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
...
#[utoipa::path(
    ...
    path = "/api/crm/objects/{object}/records",
    tag = "CRM",
    summary = "create a record on one object from a bag of field values.",
...
#[utoipa::path(
    ...
    path = "/api/crm/records/{record_id}",
    tag = "CRM",
    summary = "update one record's field values, merging by default so unmentioned fields keep what they had.",
    params(("record_id" = String, Path, description = "The record's id.")),
    request_body = UpdateRecordRequest,
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
pub(crate) async fn patch_record(
    State(state): State<AppState>,
    Path(record_id): Path<String>,
...
#[utoipa::path(
    ...
    path = "/api/crm/records/{record_id}",
    tag = "CRM",
    summary = "delete one record. Soft by default, so it can be restored.",
    params(
        ("record_id" = String, Path, description = "The record's id."),
        ("purge" = Option<bool>, Query, description = "True deletes IRREVERSIBLY, cascading over the search index, links, list entries and timeline. Leave it false unless the caller has explicitly asked for a permanent delete.")
    ),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
...
#[utoipa::path(
    post,
    path = "/api/crm/records/{record_id}/restore",
    tag = "CRM",
    summary = "bring a soft-deleted record back out of the trash.",
    params(("record_id" = String, Path, description = "The record's id.")),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
...
pub(crate) struct ValidateValuesBody {
    /// The values to check, keyed by field id or slug. Nothing is written.
    #[serde(default)]
...
#[utoipa::path(
    post,
    path = "/api/crm/objects/{object}/records/validate",
    tag = "CRM",
    summary = "check a bag of field values against an object's schema WITHOUT writing anything — the dry run before a create or update.",
    params(("object" = String, Path, description = "Object id or slug.")),
    request_body = ValidateValuesBody,
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
pub(crate) async fn validate_record_values(
    State(state): State<AppState>,
    Path(object): Path<String>,
...
#[utoipa::path(
    get,
    path = "/api/crm/records/{record_id}/links",
    tag = "CRM",
    summary = "list every relation edge on one record, in both directions — who this deal belongs to, which people work at this company.",
    params(("record_id" = String, Path, description = "The record's id.")),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
...
#[utoipa::path(
    ...
    path = "/api/crm/records/{record_id}/links",
    tag = "CRM",
    summary = "relate one record to others through a relation field — attach a person to a company, a deal to an account.",
    params(("record_id" = String, Path, description = "The record the edges start from.")),
...
#[utoipa::path(
    post,
    path = "/api/crm/records/{record_id}/unlink",
    tag = "CRM",
    summary = "remove relation edges between one record and others.",
    params(("record_id" = String, Path, description = "The record the edges start from.")),
    request_body = LinkRequest,
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
pub(crate) async fn unlink_records(
    State(state): State<AppState>,
    Path(record_id): Path<String>,
    Json(body): Json<LinkRequest>,
) -> ApiResult<Json<RecordUpdate>> {
...
#[utoipa::path(
    get,
    path = "/api/crm/records/{record_id}/related",
    tag = "CRM",
    summary = "list the records on the other end of one record's relations, as a page of full records rather than bare edges.",
    params(
        ("record_id" = String, Path, description = "The record to walk out from."),
        ("field_id" = Option<String>, Query, description = "Restrict to one relation field. Omit for every edge in both directions."),
        ("limit" = Option<usize>, Query, description = "Page size."),
        ("offset" = Option<usize>, Query, description = "How many to skip.")
    ),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
pub(crate) async fn related_records(
    State(state): State<AppState>,
    Path(record_id): Path<String>,
...
#[utoipa::path(
    post,
    path = "/api/crm/objects/{object}/duplicates",
    tag = "CRM",
    summary = "find likely duplicate records on one object, grouped into merge candidates.",
    params(("object" = String, Path, description = "Object id or slug.")),
    request_body = DuplicateScanRequest,
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
pub(crate) async fn scan_duplicates(
    ...
    Ok(Json(
        state
            .store
            .merge_candidates(&object.id, &body, limit)
            .await?,
    ))
}
...
#[utoipa::path(
    post,
    path = "/api/crm/merge/preview",
    tag = "CRM",
    summary = "dry-run a merge: what the survivor's values would become, which fields conflict, and how much history would move. Writes nothing.",
    request_body = MergePlan,
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
...
#[utoipa::path(
    ...
    path = "/api/crm/merge",
    tag = "CRM",
    summary = "merge duplicate records into one survivor, moving their timeline, links and list entries across. Preview it first.",
    request_body = MergePlan,
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
...
mod tests {
    ...
    async fn merge_mode_keeps_absent_fields_and_replace_clears_them() {
        ...
        .0;
        assert_eq!(
            merged.record.values.get("domain"),
            Some(&json!("acme.test"))
        );
    ...
    async fn a_merge_fills_blanks_but_never_overwrites() {
        ...
        assert!(
            preview.conflicts.iter().any(|c| c.field_slug == "location"),
        ...
        .0;
        assert_eq!(
            outcome.survivor.values.get("location"),
            Some(&json!("Berlin"))
        );
        assert_eq!(
