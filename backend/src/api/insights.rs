...
async fn reindex(State(state): State<AppState>) -> ApiResult<Json<Value>> {
...
async fn resolve_report_object(state: &AppState, requested: Option<&str>) -> ApiResult<Object> {
    let object_ref = requested.map(str::trim).filter(|r| !r.is_empty()).unwrap_or("deal");
    state
        .store
...
async fn tool_search(
    ...
    Ok(Json(ToolResponse::ok(summary, serde_json::to_value(&response)?)))
}
...
async fn tool_find_record(
    ...
    let field_ref = match body.field.as_deref().map(str::trim).filter(|f| !f.is_empty()) {
    ...
    let limit = state.config.clamp_limit(body.limit.or(Some(TOOL_LIMIT)));
    let mut page = find_by_field(&state, &object, &field, FilterOperator::Eq, &needle, limit).await?;
    let mut matched_exactly = true;
    if page.items.is_empty() && is_textual(field.field_type) {
        page = find_by_field(&state, &object, &field, FilterOperator::Contains, &needle, limit).await?;
...
async fn tool_get_record(
    ...
    Ok(Json(ToolResponse::ok(summary, serde_json::to_value(&detail)?)))
}
...
async fn tool_create_record(
    ...
    Ok(Json(ToolResponse::ok(summary, serde_json::to_value(&record)?)))
}
...
async fn tool_update_record(
    ...
    let summary = if update.changed.is_empty() {
        ...
        format!("“{}” was already up to date; nothing changed.", update.record.title)
    } else {
        format!(
            "Updated “{}” ({}): {}.",
            update.record.title,
...
#[derive(Debug, Default, Deserialize)]
struct ToolCreateTaskRequest {
    /// Absent for a standalone task.
    #[serde(default)]
    ...
    /// RFC-3339, or anything `models::normalize_datetime` accepts. A task with no due
    /// date is valid — it simply never trips the `task.due` sweep.
    #[serde(default)]
...
async fn tool_create_task(
    ...
    let on = match activity.record_id.as_deref() {
        Some(_) => format!(" on {}", describe_record(&state, activity.record_id.as_deref()).await?),
        None => String::new(),
    };
    Ok(Json(ToolResponse::ok(
...
mod tests {
    ...
    fn a_titleless_activity_still_gets_a_readable_row() {
        assert_eq!(activity_title("  Called Ana ", None, ActivityKind::Call), "Called Ana");
        assert_eq!(
            activity_title("", Some("\n  Left a voicemail about renewal\nmore"), ActivityKind::Call),
            "Left a voicemail about renewal"
        );
        assert_eq!(
        ...
        assert_eq!(activity_title("", Some(&long), ActivityKind::Note).chars().count(), 80);
    }
    ...
    async fn a_status_label_is_resolved_to_its_option_id_before_matching() {
        ...
        seed(
            &state,
            "deal",
            &[("name", json!("Acme renewal")), ("stage", json!("Qualified"))],
        )
        .await;
    ...
    async fn the_pipeline_sentence_prints_money_as_money_and_names_the_unassigned() {
        ...
        let report: PipelineReport = serde_json::from_value(response.data.clone().unwrap()).unwrap();
        ...
        assert!(response.summary.contains("50% win rate"), "{}", response.summary);
