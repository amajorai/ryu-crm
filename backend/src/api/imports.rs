...
async fn list_imports(
    ...
    let imports = state
        .store
        .list_imports(object_id.as_deref(), limit)
        .await?;
...
async fn emit_import_events(
    ...
    let Some(object) = state.store.get_object(object_id).await? else {
        ...
        tracing::warn!(
            object_id,
            "ryu-crm: import applied but its object is gone; no events"
        );
        return Ok(0);
    };

    let mut emitted = 0usize;
    for record_id in result.created_record_ids.iter().take(EVENT_FANOUT_LIMIT) {
...
fn infer_field_type(samples: &[String]) -> FieldType {
    ...
    if samples
        .iter()
        .any(|s| s.chars().count() > LONG_TEXT_SAMPLE_LEN)
    {
...
mod tests {
    ...
    async fn import_coerces_money_to_cents_and_dates_to_calendar_days() {
        ...
        put_mapping(
            State(state.clone()),
            Path(job.id.clone()),
            Json(straight_mapping(
                4,
                &["name", "stage", "amount", "close_date"],
            )),
        )
        .await
        .expect("mapping");
    ...
    fn column_types_are_inferred_only_when_every_sample_agrees() {
        ...
        assert_eq!(
            infer_field_type(&["42".into(), "7.5".into()]),
            FieldType::Number
        );
        ...
        assert_eq!(
            infer_field_type(&["yes".into(), "no".into()]),
            FieldType::Checkbox
        );
    ...
    async fn export_renders_labels_for_options_and_leaves_currency_in_cents() {
        ...
        assert_eq!(
            rows[1][stage_at], "Proposal",
            "the label, not the option id"
        );
