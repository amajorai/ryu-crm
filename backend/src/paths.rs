...
//! `apps-store/social/backend/src/paths.rs`, `apps-store/ugc/backend/src/paths.rs`
//! and `apps-store/mail/backend/src/paths.rs`).
//!
//! The sidecar MUST resolve the SAME data dir Core uses so it opens the SAME
//! `crm.db` the node uses. The load-bearing rule is `RYU_DIR`-env-first: Core/Kernel
...
/// The one database file this sidecar owns. A named helper rather than a
/// `join("crm.db")` at the call site, so the filename is single-sourced between
/// `main.rs` and anything that ever needs to back it up or report on it.
