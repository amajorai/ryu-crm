...
impl FieldConfig {
    ...
    pub fn resolve_option(&self, raw: &str) -> Option<&SelectOption> {
        let trimmed = raw.trim();
        self.options.iter().find(|o| o.id == trimmed).or_else(|| {
            self.options
                .iter()
    ...
    pub fn max_rating(&self) -> u8 {
        self.max_rating
            .filter(|m| *m > 0)
            .unwrap_or(Self::DEFAULT_MAX_RATING)
    }
...
pub struct CreateRecordRequest {
    /// The new record's field values, keyed by field id OR slug. Select and status
    /// values may be given as their human labels; currency as "$1,234.56".
    #[serde(default)]
    #[schema(value_type = Object)]
    pub values: ValueBag,
    /// Who is creating it, for the audit trail. Free text.
    #[serde(default)]
    pub created_by: Option<String>,
}
...
pub struct UpdateRecordRequest {
    /// The values to write, keyed by field id OR slug. In `merge` mode an explicit
    /// `null` CLEARS a field and an absent field is left alone.
    #[serde(default)]
    #[schema(value_type = Object)]
    pub values: ValueBag,
    /// `merge` (default) touches only the fields sent; `replace` makes the record
    /// exactly what was sent and clears everything else.
    #[serde(default)]
    pub mode: UpdateMode,
}
...
// No `ToSchema`: `related_records` takes this as a QUERY STRING, and its
// `#[utoipa::path]` spells the four parameters out inline. A registered-but-
// unreferenced component is dead weight in a document that is already at Core's
// per-app operation ceiling.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
...
// `no_recursion` is LOAD-BEARING, not a hint. utoipa's derive expands a field's schema
// INLINE rather than as a `$ref` when the field type is not a generic parameter, so a
// self-referential type makes `ViewFilter::schema()` call itself forever: the doc
// builds to a stack overflow at RUNTIME, with nothing wrong at compile time. It
// aborted the whole test binary before this attribute was added. The attribute stops
// the expansion one level down, which costs the document the nested branches of the
// tree — an acceptable trade, since a model composing a three-deep boolean filter is
// not the case these tools are for, and the top level still describes the shape.
...
pub struct AddListEntryRequest {
    /// The record to put on the list.
    pub record_id: String,
    /// Values for the LIST's own extra fields, keyed by list-field slug. These are a
    /// separate namespace from the record's own fields.
    #[serde(default)]
    #[schema(value_type = Object)]
    pub values: ValueBag,
}
...
pub struct UpdateListEntryRequest {
    /// Values for the LIST's own extra fields, keyed by list-field slug.
    #[serde(default)]
    #[schema(value_type = Object)]
    pub values: ValueBag,
    /// `merge` (default) touches only the fields sent; `replace` clears the rest.
    #[serde(default)]
    pub mode: UpdateMode,
}
...
pub struct ToolSearchRequest {
    /// What to search for. Free text, matched across every searchable field.
    pub query: String,
    /// Object slug to scope the search to — `company`, `person`, `deal`. Omit to
    /// search every object at once.
    #[serde(default)]
    pub object: Option<String>,
    /// Max hits to return. Defaults to 10 and is clamped to the node's ceiling.
    #[serde(default)]
    pub limit: Option<usize>,
}
...
pub struct ToolFindRecordRequest {
    /// Object slug — `company`, `person`, `deal`, or a custom object's slug.
    pub object: String,
    /// Field slug to match on, e.g. `email` or `domain`. Omit to match against the
    /// object's title field.
    #[serde(default)]
    pub field: Option<String>,
    /// The value to match. Matched exactly first; only a text-like field then falls
    /// back to a contains scan, so an email lookup stays exact.
    pub value: String,
...
pub struct ToolCreateRecordRequest {
    /// Object slug — `company`, `person`, `deal`, or any custom object's slug.
    pub object: String,
    /// The new record's field values, keyed by field SLUG. Select and status values
    /// may be given as their human labels ("Qualified"); currency as "$1,234.56";
    /// dates in any common form. The store normalises all three.
    // `ValueBag` is `serde_json::Map<String, Value>`, which utoipa has no `ToSchema`
    // impl for. `value_type = Object` renders the free-form object that is the honest
    // description anyway: the legal keys depend on the object's schema at runtime.
    #[serde(default)]
    #[schema(value_type = Object)]
    pub values: ValueBag,
}
...
pub struct ToolUpdateRecordRequest {
    ...
    pub record_id: String,
    /// Only the fields to change, keyed by field SLUG. This MERGES — fields left out
    /// keep their current values, so there is no need to resend the whole record.
    #[serde(default)]
    #[schema(value_type = Object)]
    pub values: ValueBag,
}
...
pub struct ToolLogActivityRequest {
    /// The record the entry hangs off. Required — use `create_task` for a standalone
    /// task that belongs to no record.
    pub record_id: String,
    /// What happened. Defaults to `note`. `field_change` and `stage_change` are
    /// written automatically by the store and are not accepted here.
    #[serde(default = "default_note_kind")]
    pub kind: ActivityKind,
    /// One-line headline for the timeline row. Left empty, the first line of `body`
    /// is used.
    #[serde(default)]
    pub title: String,
    /// The full text of the note / call summary / meeting notes.
    #[serde(default)]
    pub body: Option<String>,
    /// Who owns it, for a `task`. Free text.
    #[serde(default)]
    pub assignee: Option<String>,
    /// When a `task` is due, RFC-3339 or any common date form.
    #[serde(default)]
    pub due_at: Option<String>,
}
...
mod tests {
    ...
    fn ids_are_unique_within_a_millisecond() {
        let ids: std::collections::HashSet<String> = (0..1000).map(|_| new_id(ID_RECORD)).collect();
