...
impl CrmStore {
    ...
    fn migrate(conn: &Connection) -> Result<()> {
        ...
        if current < 1 {
            conn.execute_batch(V1_DDL).context("applying crm schema v1")?;
        }
...
fn sf(id: &'static str, slug: &'static str, name: &'static str, field_type: FieldType) -> SeedField {
    SeedField {
        id,
        slug,
        name,
...
fn seed_objects() -> Vec<(Object, Vec<SeedField>, Vec<View>)> {
    let now = now_rfc3339();
    let object = |id: &str, slug: &str, singular: &str, plural: &str, icon: &str, title: &str, position: i64| Object {
        id: id.to_string(),
        slug: slug.to_string(),
    ...
    let view = |id: &str, object_id: &str, name: &str, kind: ViewKind, visible: &[&str], group_by: Option<&str>, is_default: bool, position: i64| View {
        id: id.to_string(),
    ...
    vec![
        (
            object(OBJ_COMPANY, "company", "Company", "Companies", "building-2", FLD_COMPANY_NAME, 0),
            vec![
                ...
                sf(FLD_COMPANY_STATUS, "status", "Status", FieldType::Status).options(vec![
                    ...
                    SelectOption::new(OPT_COMPANY_STATUS_PROSPECT, "Prospect", 1).with_color("blue"),
                    SelectOption::new(OPT_COMPANY_STATUS_CUSTOMER, "Customer", 2).with_color("green").won(),
                    SelectOption::new(OPT_COMPANY_STATUS_CHURNED, "Churned", 3).with_color("red").lost(),
                ]),
                sf(FLD_COMPANY_INDUSTRY, "industry", "Industry", FieldType::Select).options(vec![
                ...
                sf(FLD_COMPANY_EMPLOYEES, "employees", "Employees", FieldType::Number),
                sf(FLD_COMPANY_ARR, "arr", "ARR", FieldType::Currency).currency("USD"),
                sf(FLD_COMPANY_PHONE, "phone", "Phone", FieldType::Phone),
                sf(FLD_COMPANY_LOCATION, "location", "Location", FieldType::Text),
                sf(FLD_COMPANY_DESCRIPTION, "description", "Description", FieldType::LongText),
            ...
            vec![view(
                ...
                "All companies",
                ViewKind::Table,
                &[FLD_COMPANY_NAME, FLD_COMPANY_DOMAIN, FLD_COMPANY_STATUS, FLD_COMPANY_INDUSTRY, FLD_COMPANY_ARR, FLD_COMPANY_OWNER],
                None,
                true,
                0,
            )],
        ...
        (
            object(OBJ_PERSON, "person", "Person", "People", "user-round", FLD_PERSON_NAME, 1),
            vec![
                ...
                sf(FLD_PERSON_JOB_TITLE, "job_title", "Job title", FieldType::Text),
                sf(FLD_PERSON_COMPANY, "company", "Company", FieldType::Relation)
            ...
            vec![view(
                ...
                "All people",
                ViewKind::Table,
                &[FLD_PERSON_NAME, FLD_PERSON_EMAIL, FLD_PERSON_JOB_TITLE, FLD_PERSON_COMPANY, FLD_PERSON_OWNER],
                None,
                true,
                0,
            )],
        ...
        (
            object(OBJ_DEAL, "deal", "Deal", "Deals", "target", FLD_DEAL_NAME, 2),
            vec![
                ...
                sf(FLD_DEAL_STAGE, "stage", "Stage", FieldType::Status).required().options(vec![
                    SelectOption::new(OPT_DEAL_STAGE_LEAD, "Lead", 0).with_color("slate"),
                    SelectOption::new(OPT_DEAL_STAGE_QUALIFIED, "Qualified", 1).with_color("blue"),
                    SelectOption::new(OPT_DEAL_STAGE_PROPOSAL, "Proposal", 2).with_color("indigo"),
                    SelectOption::new(OPT_DEAL_STAGE_NEGOTIATION, "Negotiation", 3).with_color("amber"),
                    SelectOption::new(OPT_DEAL_STAGE_WON, "Won", 4).with_color("green").won(),
                    SelectOption::new(OPT_DEAL_STAGE_LOST, "Lost", 5).with_color("red").lost(),
                ]),
                sf(FLD_DEAL_AMOUNT, "amount", "Amount", FieldType::Currency).currency("USD"),
                sf(FLD_DEAL_PROBABILITY, "probability", "Probability", FieldType::Percent),
                sf(FLD_DEAL_CLOSE_DATE, "close_date", "Close date", FieldType::Date),
                sf(FLD_DEAL_COMPANY, "company", "Company", FieldType::Relation)
                    .relation(OBJ_COMPANY, false, "Deals"),
                ...
                sf(FLD_DEAL_DESCRIPTION, "description", "Description", FieldType::LongText),
            ],
            vec![
                view(
                    VIEW_DEAL_ALL,
                    ...
                    "All deals",
                    ViewKind::Table,
                    &[FLD_DEAL_NAME, FLD_DEAL_STAGE, FLD_DEAL_AMOUNT, FLD_DEAL_CLOSE_DATE, FLD_DEAL_COMPANY, FLD_DEAL_OWNER],
                    None,
                    true,
                    0,
                ),
                view(
                    ...
                    "Pipeline",
                    ViewKind::Board,
                    &[FLD_DEAL_NAME, FLD_DEAL_AMOUNT, FLD_DEAL_CLOSE_DATE, FLD_DEAL_COMPANY],
                    Some(FLD_DEAL_STAGE),
                    false,
                    1,
                ),
        ...
        (
            object(OBJ_NOTE, "note", "Note", "Notes", "sticky-note", FLD_NOTE_SUBJECT, 3),
            vec![
        ...
        (
            object(OBJ_TASK, "task", "Task", "Tasks", "circle-check", FLD_TASK_NAME, 4),
            vec![
                ...
                sf(FLD_TASK_STATUS, "status", "Status", FieldType::Status).options(vec![
                    ...
                    SelectOption::new(OPT_TASK_STATUS_IN_PROGRESS, "In progress", 1).with_color("blue"),
                    SelectOption::new(OPT_TASK_STATUS_DONE, "Done", 2).with_color("green").won(),
                    SelectOption::new(OPT_TASK_STATUS_CANCELLED, "Cancelled", 3).with_color("red").lost(),
                ]),
            ...
            vec![
                view(
                    ...
                    "All tasks",
                    ViewKind::Table,
                    &[FLD_TASK_NAME, FLD_TASK_STATUS, FLD_TASK_ASSIGNEE, FLD_TASK_DUE_DATE, FLD_TASK_PRIORITY],
                    None,
                    true,
                    0,
                ),
                view(
                    ...
                    "Task board",
                    ViewKind::Board,
                    &[FLD_TASK_NAME, FLD_TASK_ASSIGNEE, FLD_TASK_DUE_DATE, FLD_TASK_PRIORITY],
...
const COLS_FIELD: &str = "id, object_id, list_id, slug, name, field_type, config, description, \
                          is_required, is_unique, is_system, position, created_at, updated_at";
const COLS_RECORD: &str = "id, object_id, title, data, deleted_at, created_by, created_at, updated_at";
...
fn load_list(conn: &Connection, list_id: &str) -> Result<Option<List>> {
    ...
    Ok(conn.query_row(&sql, params![list_id], row_to_list).optional()?)
}
...
fn next_position(conn: &Connection, sql: &str, key: &str) -> Result<i64> {
    let max: Option<i64> = conn.query_row(sql, params![key], |r| r.get(0)).optional()?.flatten();
    ...
    match field.field_type {
        ...
        FieldType::Email => {
            ...
            let ok = !lowered.contains(char::is_whitespace)
                ...
                && lowered
                    .split_once('@')
                    .is_some_and(|(user, host)| !user.is_empty() && host.contains('.') && !host.starts_with('.') && !host.ends_with('.'));
            if !ok {
        ...
        FieldType::Rating => {
            ...
            if rounded < 0 || rounded > max {
                return Err(out_of_range(&format!("a rating must be between 0 and {max}")));
            }
...
fn validate_bag(
    ...
    for field in fields.iter().filter(|f| f.field_type == FieldType::Relation) {
    ...
    for field in fields.iter().filter(|f| f.is_unique) {
        ...
        if let Some(other) = clash {
            out.errors.push(FieldValidationError::coded(
                ...
                format!("another record ({other}) already has this {}", field.name.to_lowercase()),
            ));
        }
    }
...
fn compute_title(object: &Object, fields: &[Field], values: &ValueBag) -> String {
    let from_field = |field: &Field| -> Option<String> {
        values.get(&field.slug).and_then(as_text)
    };
    ...
    for field in fields.iter().filter(|f| {
        matches!(f.field_type, FieldType::Text | FieldType::Email)
    }) {
...
fn fts_reindex(conn: &Connection, record_id: &str, title: &str, body: &str) -> Result<()> {
    let rowid: Option<i64> = conn
        .query_row("SELECT rowid FROM records WHERE id = ?1", params![record_id], |r| r.get(0))
        .optional()?;
...
impl CrmStore {
    ...
    pub async fn schema(&self) -> Result<SchemaResponse> {
        ...
        let lists = {
            let sql = format!("SELECT {COLS_LIST} FROM lists ORDER BY position ASC, created_at ASC");
            let mut stmt = conn.prepare(&sql)?;
            let rows = stmt.query_map([], row_to_list)?;
            rows.collect::<rusqlite::Result<Vec<_>>>()?
        };
        Ok(SchemaResponse { objects: out, lists })
    }
    ...
    pub async fn update_object(
        ...
        conn.execute(
            ...
            params![
                existing.id,
                req.singular.as_deref().map(str::trim).filter(|s| !s.is_empty()).unwrap_or(&existing.singular),
                req.plural.as_deref().map(str::trim).filter(|s| !s.is_empty()).unwrap_or(&existing.plural),
                req.icon.as_ref().or(existing.icon.as_ref()),
                req.description.as_ref().or(existing.description.as_ref()),
                req.title_field_id.as_ref().or(existing.title_field_id.as_ref()),
    ...
    pub async fn delete_object(&self, object_id: &str) -> Result<bool> {
        ...
        tx.execute("DELETE FROM activities WHERE object_id = ?1", params![object.id])?;
...
impl CrmStore {
    ...
    pub async fn create_field(
        ...
        let taken = match list_id {
            Some(list) => load_list_fields(&conn, list)?.iter().any(|f| f.slug == slug),
            None => load_fields(&conn, &object.id)?.iter().any(|f| f.slug == slug),
        };
        if taken {
            return Ok(Err(vec![FieldValidationError::coded(
                "",
    ...
    /// models module docs.
    pub async fn update_field(&self, field_id: &str, req: &UpdateFieldRequest) -> Validated<Option<Field>> {
        let conn = self.conn.lock().await;
        let Some(existing) = load_field(&conn, field_id)? else {
            return Ok(Ok(None));
        };
        let mut config = req.config.clone().unwrap_or_else(|| existing.config.clone());
    ...
    pub async fn delete_field(&self, field_id: &str) -> Result<bool> {
        ...
        tx.execute("DELETE FROM record_links WHERE field_id = ?1", params![field.id])?;
        match field.list_id.as_deref() {
            ...
            None => {
                tx.execute(
                    &format!("UPDATE records SET data = json_remove(data, '$.{}') WHERE object_id = ?1", field.slug),
    ...
    /// order after them.
    pub async fn reorder_fields(&self, object_id: &str, list_id: Option<&str>, ids: &[String]) -> Result<()> {
        let mut conn = self.conn.lock().await;
        let tx = conn.transaction()?;
        let now = now_rfc3339();
...
fn validate_config(slug: &str, field_type: FieldType, config: &FieldConfig) -> Option<FieldValidationError> {
...
fn reindex_object(conn: &Connection, object_id: &str) -> Result<usize> {
    ...
    for record in &records {
        fts_reindex(conn, &record.id, &record.title, &fts_body(&fields, &record.values))?;
    }
    Ok(records.len())
}
...
fn value_expr(index: &HashMap<String, Field>, key: &str, alias: &str) -> Option<(String, Option<Field>)> {
...
fn build_condition(
    ...
    let any_of = |values: &[Value], params: &mut SqlParams| -> String {
        ...
        let placeholders = values
            ...
            .join(", ");
        if multi {
            format!("EXISTS (SELECT 1 FROM json_each({expr}) je WHERE je.value IN ({placeholders}))")
        } else {
    ...
    let sql = match condition.op {
        ...
        FilterOperator::NotContains => {
            scalar(params);
            format!("({expr} IS NULL OR lower(CAST({expr} AS TEXT)) NOT LIKE '%' || lower(?) || '%')")
        }
        ...
        FilterOperator::IsAnyOf => {
            let values = condition.value.as_array().cloned().unwrap_or_else(|| vec![condition.value.clone()]);
...
fn build_filter(
    ...
    match filter {
        ViewFilter::And { filters } | ViewFilter::Or { filters } => {
            let joiner = if matches!(filter, ViewFilter::And { .. }) { " AND " } else { " OR " };
...
impl CrmStore {
    ...
    pub async fn purge_record(&self, record_id: &str) -> Result<bool> {
        ...
        tx.execute("DELETE FROM list_entries WHERE record_id = ?1", params![record_id])?;
        tx.execute("DELETE FROM activities WHERE record_id = ?1", params![record_id])?;
    ...
    pub async fn query_records(&self, query: &RecordQuery, limit: usize, offset: usize) -> Result<RecordPage> {
        let conn = self.conn.lock().await;
        ...
        let total: i64 = conn.query_row(&count_sql, params_from_iter(params.clone()), |r| r.get(0))?;
    ...
    pub async fn validate_values(
        ...
        let fields = load_fields(&conn, &object.id)?;
        validate_bag(&conn, &object.id, &fields, values, partial, exclude_record_id)
    }
}
...
fn build_record_where(
    ...
    if let Some(expression) = query.search.as_deref().and_then(fts_match_expression) {
        clauses.push("r.rowid IN (SELECT rowid FROM records_fts WHERE records_fts MATCH ?)".to_string());
...
fn link_views(conn: &Connection, record_id: &str) -> Result<Vec<RecordLinkView>> {
    ...
    for link in links {
        ...
        let other_id = if outgoing { &link.target_record_id } else { &link.source_record_id };
        let other_object = if outgoing { &link.target_object_id } else { &link.source_object_id };
        let title: Option<String> = conn
            .query_row("SELECT title FROM records WHERE id = ?1 AND deleted_at IS NULL", params![other_id], |r| r.get(0))
            .optional()?;
        let Some(title) = title else { continue };
        let field = load_field(conn, &link.field_id)?;
        let label = if outgoing {
            field.as_ref().map(|f| f.name.clone()).unwrap_or_else(|| "Related".to_string())
        } else {
        ...
        out.push(RecordLinkView {
            ...
            label,
            direction: if outgoing { LinkDirection::Outgoing } else { LinkDirection::Incoming },
...
impl CrmStore {
    ...
    pub async fn link_records(&self, record_id: &str, req: &LinkRequest) -> Validated<Option<RecordUpdate>> {
        self.mutate_links(record_id, req, true).await
    }

    pub async fn unlink_records(&self, record_id: &str, req: &LinkRequest) -> Validated<Option<RecordUpdate>> {
    ...
    async fn mutate_links(
        ...
        let mut targets: Vec<String> = existing.values.get(&field.slug).map(as_list).unwrap_or_default();
...
impl CrmStore {
    ...
    pub async fn merge_candidates(
        ...
        let conn = self.conn.lock().await;
        let Some(object) = load_object(&conn, object_id)? else {
            return Ok(DuplicateScanResponse { candidates: Vec::new(), field_ids: Vec::new() });
        };
        let fields = load_fields(&conn, &object.id)?;
        let index = field_index(&fields);
        ...
        for field in &chosen {
            ...
            for row in rows {
                ...
                for id in &record_ids {
                    let title: Option<String> = conn
                        .query_row("SELECT title FROM records WHERE id = ?1", params![id], |r| r.get(0))
                        .optional()?;
    ...
    pub async fn plan_merge(&self, plan: &MergePlan) -> Result<Option<MergePreview>> {
        let conn = self.conn.lock().await;
        let Some((survivor, losers, fields, resolved, conflicts)) = resolve_merge(&conn, plan)? else {
            return Ok(None);
        };
        let _ = fields;
    ...
    pub async fn merge_records(&self, plan: &MergePlan) -> Result<Option<MergeOutcome>> {
        ...
        for loser in &losers {
            ...
            tx.execute("DELETE FROM list_entries WHERE record_id = ?1", params![loser.id])?;
...
fn resolve_merge(
    conn: &Connection,
    plan: &MergePlan,
) -> Result<Option<(Record, Vec<Record>, Vec<Field>, ValueBag, Vec<MergeConflict>)>> {
    ...
    for field in &fields {
        let survivor_value = survivor.values.get(&field.slug).cloned().unwrap_or(Value::Null);
        let differing: Vec<MergeLoserValue> = losers
            .iter()
            .filter_map(|loser| {
                let value = loser.values.get(&field.slug).cloned().unwrap_or(Value::Null);
        ...
        let chosen = match source {
            ...
            None => {
                if is_empty_value(&survivor_value) {
                    differing.first().map(|l| l.value.clone()).unwrap_or(Value::Null)
                } else {
...
fn load_view(conn: &Connection, view_id: &str) -> Result<Option<View>> {
    ...
    Ok(conn.query_row(&sql, params![view_id], row_to_view).optional()?)
}
...
impl CrmStore {
    ...
    pub async fn create_view(&self, object_id: &str, req: &CreateViewRequest) -> Result<View> {
        ...
        let position = next_position(&tx, "SELECT MAX(position) FROM views WHERE object_id = ?1", &object.id)?;
        if req.is_default {
            tx.execute("UPDATE views SET is_default = 0 WHERE object_id = ?1", params![object.id])?;
        }
        tx.execute(
            "INSERT INTO views
               (id, object_id, name, kind, filter, sorts, visible_fields, group_by_field_id,
    ...
    pub async fn update_view(&self, view_id: &str, req: &UpdateViewRequest) -> Result<Option<View>> {
        let conn = self.conn.lock().await;
        ...
        conn.execute(
            ...
            params![
                ...
                req.filter.as_ref().or(existing.filter.as_ref()).map(encode_json),
                encode_json(req.sorts.as_ref().unwrap_or(&existing.sorts)),
                encode_json(req.visible_field_ids.as_ref().unwrap_or(&existing.visible_field_ids)),
                req.group_by_field_id.as_ref().or(existing.group_by_field_id.as_ref()),
                req.position.unwrap_or(existing.position),
                now
            ],
        )?;
    ...
    pub async fn set_default_view(&self, view_id: &str) -> Result<bool> {
        ...
        let tx = conn.transaction()?;
        tx.execute("UPDATE views SET is_default = 0 WHERE object_id = ?1", params![view.object_id])?;
        tx.execute("UPDATE views SET is_default = 1 WHERE id = ?1", params![view.id])?;
        tx.commit()?;
        Ok(true)
    }
    ...
    pub async fn run_view(
        ...
        let filter = match (view.filter.clone(), overrides.filter.clone()) {
            (Some(saved), Some(extra)) => Some(ViewFilter::And { filters: vec![saved, extra] }),
            (Some(saved), None) => Some(saved),
            (None, extra) => extra,
        };
        let sorts = overrides.sorts.clone().unwrap_or_else(|| view.sorts.clone());
...
fn build_board_groups(
    ...
    for (option, position) in buckets {
        ...
        groups.push(BoardGroup {
            ...
            label: option.as_ref().map(|o| o.label.clone()).unwrap_or_else(|| "No value".to_string()),
...
impl CrmStore {
    ...
    pub async fn create_list(&self, req: &CreateListRequest) -> Result<List> {
        ...
        let position = next_position(&conn, "SELECT MAX(position) FROM lists WHERE object_id = ?1", &object.id)?;
        conn.execute(
    ...
    pub async fn update_list(&self, list_id: &str, req: &UpdateListRequest) -> Result<Option<List>> {
        let conn = self.conn.lock().await;
    ...
    pub async fn delete_list(&self, list_id: &str) -> Result<bool> {
        ...
        let tx = conn.transaction()?;
        tx.execute("DELETE FROM list_entries WHERE list_id = ?1", params![list_id])?;
    ...
    pub async fn add_list_entry(&self, list_id: &str, req: &AddListEntryRequest) -> Validated<ListEntry> {
        let conn = self.conn.lock().await;
        ...
        let validated = validate_bag(&conn, &list.object_id, &list_fields, &req.values, false, None)?;
        if !validated.is_ok() {
            return Ok(Err(validated.errors));
        }
        let mut values = validated.values;
        ...
        let position = next_position(&conn, "SELECT MAX(position) FROM list_entries WHERE list_id = ?1", &list.id)?;
        conn.execute(
    ...
    pub async fn update_list_entry(
        ...
        let Some(existing) = conn.query_row(&sql, params![entry_id], row_to_list_entry).optional()? else {
            return Ok(Ok(None));
        };
        ...
        let list_fields = load_list_fields(&conn, &list.id)?;
        let partial = req.mode == UpdateMode::Merge;
        let validated = validate_bag(&conn, &list.object_id, &list_fields, &req.values, partial, None)?;
        if !validated.is_ok() {
            return Ok(Err(validated.errors));
        }
        let mut next = match req.mode {
    ...
    pub async fn query_list_entries(
        ...
        let mut clauses = vec!["e.list_id = ?".to_string(), "r.deleted_at IS NULL".to_string()];
        if let Some(expression) = query.search.as_deref().and_then(fts_match_expression) {
            clauses.push("r.rowid IN (SELECT rowid FROM records_fts WHERE records_fts MATCH ?)".to_string());
            params.push(rusqlite::types::Value::Text(expression));
        }
        if let Some(filter) = query.filter.as_ref().filter(|f| !f.is_empty()) {
            ...
            clauses.push(build_scoped_filter(filter, &list_index, &record_index, &mut params));
        }
        ...
        let rows = stmt.query_map(params_from_iter(params), |row| {
            ...
            let record = Record {
                ...
                object_id: row.get(8)?,
                title: row.get(9)?,
                values: serde_json::from_str::<ValueBag>(&row.get::<_, String>(10)?).unwrap_or_default(),
    ...
    match filter {
        ViewFilter::And { filters } | ViewFilter::Or { filters } => {
            ...
            let parts: Vec<String> = filters
                ...
                .filter(|p| p != "1")
                .collect();
            if parts.is_empty() { "1".to_string() } else { format!("({})", parts.join(joiner)) }
        }
        ViewFilter::Not { filter } => {
            let inner = build_scoped_filter(filter, list_index, record_index, params);
            if inner == "1" { "1".to_string() } else { format!("NOT ({inner})") }
        }
        ViewFilter::Condition(condition) => {
            if list_index.contains_key(&condition.field_id) {
...
fn build_scoped_order_by(
    ...
    for sort in sorts.iter().take(MAX_SORTS) {
        ...
        let Some((expr, field)) = resolved else { continue };
        let collate = if field.as_ref().is_none_or(|f| !f.field_type.is_numeric()) { " COLLATE NOCASE" } else { "" };
        parts.push(format!("({expr} IS NULL) ASC, {expr}{collate} {}", sort.direction.as_sql()));
    }
...
impl CrmStore {
    pub async fn get_activity(&self, activity_id: &str) -> Result<Option<Activity>> {
        ...
        Ok(conn.query_row(&sql, params![activity_id], row_to_activity).optional()?)
    }
    ...
    pub async fn create_activity(&self, req: &CreateActivityRequest) -> Validated<Activity> {
        if !req.kind.is_user_authored() {
            return Ok(Err(vec![FieldValidationError::coded(
                ...
                "kind",
                ValidationCode::Invalid,
                format!("\"{}\" entries are written automatically and cannot be created directly", req.kind.as_str()),
            )]));
        }
        let conn = self.conn.lock().await;
    ...
    pub async fn update_activity(
        ...
        let conn = self.conn.lock().await;
        let sql = format!("SELECT {COLS_ACTIVITY} FROM activities WHERE id = ?1");
        let Some(existing) = conn.query_row(&sql, params![activity_id], row_to_activity).optional()? else {
            return Ok(Ok(None));
        };
        ...
        conn.execute(
            ...
            params![
                existing.id,
                req.title.as_deref().map(str::trim).unwrap_or(&existing.title),
                req.body.as_ref().or(existing.body.as_ref()),
                req.assignee.as_ref().or(existing.assignee.as_ref()),
                due_at,
                completed_at,
                req.metadata.as_ref().map(encode_json).or_else(|| existing.metadata.as_ref().map(encode_json)),
                now
            ],
        )?;
        Ok(Ok(conn.query_row(&sql, params![existing.id], row_to_activity).optional()?))
    }
    ...
    pub async fn complete_task(&self, activity_id: &str, completed: bool) -> Result<Option<Activity>> {
        let conn = self.conn.lock().await;
        let now = now_rfc3339();
        conn.execute(
    ...
    pub async fn list_tasks(&self, query: &TaskQuery, limit: usize, offset: usize) -> Result<ActivityPage> {
        let conn = self.conn.lock().await;
        let now = now_rfc3339();
        ...
        match query.filter {
            ...
            TaskFilter::Overdue => {
                clauses.push("a.completed_at IS NULL AND a.due_at IS NOT NULL AND a.due_at <= ?".to_string());
...
impl CrmStore {
    ...
    /// to `records`.
    pub async fn create_import(&self, req: &CreateImportRequest, max_bytes: usize) -> Validated<ImportJob> {
        ...
        let has_header = req.has_header.unwrap_or_else(|| looks_like_header(&rows[0]));
        ...
        for index in 0..width {
            let name = if has_header {
                rows[0].get(index).map(|c| c.trim().to_string()).filter(|c| !c.is_empty())
            } else {
                None
            }
    ...
    pub async fn get_import(&self, import_id: &str) -> Result<Option<ImportJob>> {
        let conn = self.conn.lock().await;
        let sql = format!("SELECT {COLS_IMPORT} FROM import_jobs WHERE id = ?1");
        Ok(conn.query_row(&sql, params![import_id], row_to_import).optional()?)
    }

    pub async fn list_imports(&self, object_id: Option<&str>, limit: usize) -> Result<Vec<ImportJob>> {
        let conn = self.conn.lock().await;
        let sql = format!(
    ...
    pub async fn dry_run_import(&self, import_id: &str) -> Result<Option<ImportPreview>> {
        ...
        preview.conflicts = conflicts.into_iter().take(ImportPreview::CONFLICT_LIMIT).collect();
        ...
        for plan in plans {
            match plan.action {
                ...
                ImportAction::Create => {
                    ...
                    prune_nulls(&mut values);
                    let record = insert_record(&tx, &object, &fields, values, Some(&format!("import:{}", job.id)))?;
                    result.created += 1;
...
fn plan_import(
    ...
    let data_rows: &[Vec<String>] = if job.has_header && !rows.is_empty() { &rows[1..] } else { &rows[..] };
    ...
    for (row_index, row) in data_rows.iter().enumerate() {
        ...
        for (column_index, field) in &mapping {
            let Some(cell) = row.get(*column_index) else { continue };
        ...
        let (action, record_id) = match (&matched, job.dedupe.strategy) {
            ...
            (Some(existing), DedupeStrategy::Skip) => (ImportAction::Skip, Some(existing.id.clone())),
...
fn find_import_match(
    ...
    let mut clauses = vec!["object_id = ?1".to_string(), "deleted_at IS NULL".to_string()];
...
impl CrmStore {
    ...
    pub async fn search(&self, query: &SearchQuery, limit: usize, offset: usize) -> Result<SearchResponse> {
        let conn = self.conn.lock().await;
...
impl CrmStore {
    ...
    pub async fn pipeline_report(&self, req: &PipelineRequest) -> Result<Option<PipelineReport>> {
        ...
        let value_field = req
            ...
            .or_else(|| fields.iter().find(|f| f.field_type == FieldType::Currency).cloned());
        ...
        Ok(Some(PipelineReport {
            ...
            win_rate: if closed > 0 { won_count as f64 / closed as f64 } else { 0.0 },
        }))
    }
    ...
    pub async fn funnel_report(&self, req: &FunnelRequest) -> Result<Option<FunnelReport>> {
        ...
        let mut traces: BTreeMap<String, Vec<(String, String)>> = BTreeMap::new();
        {
            ...
            for row in rows {
                ...
                if entry.is_empty() {
                    ...
                    if let Some(from) = from_raw.and_then(|v| serde_json::from_str::<Value>(&v).ok()).and_then(|v| v.as_str().map(str::to_string)) {
                        entry.push((from, at.clone()));
                    }
                }
                if let Some(to) = to_raw.and_then(|v| serde_json::from_str::<Value>(&v).ok()).and_then(|v| v.as_str().map(str::to_string)) {
                    entry.push((to, at));
                }
            }
        }
        ...
        // straight into it. Omitting them makes a young pipeline look empty.
        {
            ...
            for row in rows {
                ...
                let Some(stage) = stage.filter(|s| !s.is_empty()) else { continue };
                traces.entry(record_id).or_insert_with(|| vec![(stage, created_at)]);
            }
        }
        ...
        for option in &options {
            ...
            for trace in traces.values() {
                ...
                if let (Some((_, from)), Some((_, to))) = (trace.get(at_index), trace.get(at_index + 1)) {
                    if let (Ok(a), Ok(b)) = (
            ...
            steps.push(FunnelStep {
                ...
                entered,
                advanced,
                conversion_rate: if entered > 0 { advanced as f64 / entered as f64 } else { 0.0 },
    ...
    pub async fn summary(&self, recent_limit: usize) -> Result<CrmSummary> {
        ...
        let pipeline = self.pipeline_report(&PipelineRequest {
            include_closed: true,
            ..Default::default()
        })
        .await?;
...
mod tests {
    ...
    async fn currency_is_cents_when_integer_and_major_units_otherwise() {
        let store = store().await;
        let panel = make_record(&store, "deal", &[("name", json!("A")), ("stage", json!("Lead")), ("amount", json!(12345))]).await;
        assert_eq!(panel.values["amount"], json!(12345));
        let csv = make_record(&store, "deal", &[("name", json!("B")), ("stage", json!("Lead")), ("amount", json!("$1,234.56"))]).await;
        assert_eq!(csv.values["amount"], json!(123456));
        let agent = make_record(&store, "deal", &[("name", json!("C")), ("stage", json!("Lead")), ("amount", json!(99.5))]).await;
    ...
    async fn select_values_accept_labels_and_normalize_to_option_ids() {
        let store = store().await;
        let deal = make_record(&store, "deal", &[("name", json!("Acme")), ("stage", json!("Proposal"))]).await;
    ...
    async fn required_and_unknown_fields_are_reported_together_not_one_at_a_time() {
        ...
        // have reported one.
        assert!(errors.iter().filter(|e| e.code == ValidationCode::Required).count() >= 2);
    }

    #[tokio::test]
    async fn unique_fields_reject_a_second_record_but_tolerate_blanks_and_self() {
        let store = store().await;
        let first = make_record(&store, "person", &[("name", json!("Jane")), ("email", json!("jane@acme.com"))]).await;
        let clash = store
            .create_record(
                "person",
                &CreateRecordRequest {
        ...
        let update = store
            .update_record(
                &first.id,
                &UpdateRecordRequest {
                    values: bag(&[("email", json!("jane@acme.com")), ("job_title", json!("CTO"))]),
        ...
        assert!(update.unwrap().changed.iter().any(|c| c.field_slug == "job_title"));
    }
    ...
    async fn updates_diff_status_fields_and_write_a_stage_change_activity() {
        let store = store().await;
        let deal = make_record(&store, "deal", &[("name", json!("Acme")), ("stage", json!("Lead"))]).await;
        let update = store
            .update_record(
                &deal.id,
                &UpdateRecordRequest {
        ...
        let timeline = store
            ...
            .await
            .unwrap();
        assert!(timeline.items.iter().any(|a| a.kind == ActivityKind::StageChange));
        assert!(timeline.items.iter().any(|a| a.kind == ActivityKind::FieldChange));
    ...
    async fn filters_and_sorts_compose_over_the_value_bag() {
        ...
        for (name, stage, amount) in [
            ...
            make_record(
                &store,
                "deal",
                &[("name", json!(name)), ("stage", json!(stage)), ("amount", json!(amount))],
            )
            .await;
        }
        let page = store
    ...
    async fn search_finds_records_and_survives_operator_characters() {
        let store = store().await;
        make_record(&store, "company", &[("name", json!("Acme Industries")), ("description", json!("A widget manufacturer"))]).await;
        make_record(&store, "company", &[("name", json!("Globex"))]).await;

        let hits = store
            .search(&SearchQuery { query: "widget".into(), ..Default::default() }, 10, 0)
            .await
            .unwrap();
        ...
        let prefix = store
            .search(&SearchQuery { query: "acm".into(), ..Default::default() }, 10, 0)
            .await
            .unwrap();
        ...
        let hostile = store
            .search(&SearchQuery { query: "acme OR \"".into(), ..Default::default() }, 10, 0)
            .await
            .unwrap();
    ...
    async fn soft_delete_hides_a_record_from_search_and_queries_until_restored() {
        ...
        let visible = store
            .query_records(&RecordQuery { object_id: "company".into(), ..Default::default() }, 10, 0)
            .await
            .unwrap();
        assert_eq!(visible.total, 0);
        assert_eq!(
            store.search(&SearchQuery { query: "Initech".into(), ..Default::default() }, 10, 0).await.unwrap().total,
            0
        );
        assert!(store.restore_record(&company.id).await.unwrap());
        assert_eq!(
            store.query_records(&RecordQuery { object_id: "company".into(), ..Default::default() }, 10, 0).await.unwrap().total,
            1
        );
    }
    ...
    async fn merge_fills_blanks_moves_history_and_never_silently_overwrites() {
        let store = store().await;
        let survivor = make_record(&store, "person", &[("name", json!("Jane Doe")), ("email", json!("jane@acme.com"))]).await;
        let loser = make_record(&store, "person", &[("name", json!("J. Doe")), ("phone", json!("+1 555 0100"))]).await;
        store
        ...
        assert!(store.get_record(&loser.id).await.unwrap().unwrap().deleted_at.is_some());
    }
    ...
    async fn duplicate_scan_finds_shared_emails_and_says_which_field_it_used() {
        ...
        make_record(&store, "company", &[("name", json!("Acme")), ("domain", json!("acme.com"))]).await;
        make_record(&store, "company", &[("name", json!("ACME Inc")), ("domain", json!("ACME.com"))]).await;
        let scan = store
            .merge_candidates(
                "company",
    ...
    async fn csv_import_previews_then_applies_the_same_plan() {
        let store = store().await;
        make_record(&store, "person", &[("name", json!("Jane Doe")), ("email", json!("jane@acme.com"))]).await;
        ...
        assert_eq!(job.columns[0].suggested_field_id.as_deref(), Some(FLD_PERSON_NAME));
        assert_eq!(job.columns[2].suggested_field_id.as_deref(), Some(FLD_PERSON_JOB_TITLE));
        ...
        assert_eq!(preview.update_count, 1, "the existing Jane gets her blank job title filled");
        ...
        assert_eq!(
            store.query_records(&RecordQuery { object_id: "person".into(), ..Default::default() }, 50, 0).await.unwrap().total,
            3
        );
        // The quoted cell survived the parser intact.
        let hits = store.search(&SearchQuery { query: "Quoted".into(), ..Default::default() }, 10, 0).await.unwrap();
        assert_eq!(hits.total, 1);
    ...
    async fn import_rows_that_fail_validation_do_not_block_the_rest() {
        ...
        // worse than none.
        assert!(preview.samples.iter().any(|s| s.action == ImportAction::Error));
        let result = store.apply_import(&job.id).await.unwrap().unwrap();
    ...
    async fn lists_carry_their_own_fields_separate_from_the_records() {
        ...
        let list_field = store
            .create_field(
                ...
                &CreateFieldRequest {
                    ...
                    config: FieldConfig {
                        options: vec![SelectOption::new("", "Shortlisted", 0), SelectOption::new("", "Dropped", 1)],
                        ..Default::default()
                    },
                    ..Default::default()
                },
        ...
        let entry = store
            ...
            .unwrap()
            .unwrap();
        assert_eq!(entry.values["stage"], json!(list_field.config.options[0].id));

        let page = store
            .query_list_entries(&ListEntryQuery { list_id: list.id.clone(), ..Default::default() }, 10, 0)
            .await
            .unwrap();
        assert_eq!(page.total, 1);
        assert_eq!(page.items[0].record.title, "Acme");
        // The record's OWN stage is untouched by the list's.
        assert_eq!(page.items[0].record.values["stage"], json!(OPT_DEAL_STAGE_LEAD));
    ...
    async fn board_views_group_by_status_and_always_keep_a_no_value_column() {
        let store = store().await;
        make_record(&store, "deal", &[("name", json!("A")), ("stage", json!("Lead")), ("amount", json!(1000))]).await;
        make_record(&store, "deal", &[("name", json!("B")), ("stage", json!("Won")), ("amount", json!(2000))]).await;
        let result = store
        ...
        let won = groups.iter().find(|g| g.option_id.as_deref() == Some(OPT_DEAL_STAGE_WON)).unwrap();
    ...
    async fn pipeline_report_counts_unassigned_rather_than_dropping_it() {
        ...
        make_record(&store, "task", &[("name", json!("A")), ("status", json!("Done"))]).await;
        make_record(&store, "task", &[("name", json!("B")), ("status", json!("Cancelled"))]).await;
    ...
    async fn funnel_report_credits_records_created_straight_into_a_stage() {
        let store = store().await;
        let deal = make_record(&store, "deal", &[("name", json!("A")), ("stage", json!("Lead"))]).await;
        store
            .update_record(
                &deal.id,
                &UpdateRecordRequest {
            ...
            .unwrap()
            .unwrap();
        make_record(&store, "deal", &[("name", json!("B")), ("stage", json!("Lead"))]).await;

        let report = store.funnel_report(&FunnelRequest::default()).await.unwrap().unwrap();
        let lead = report.steps.iter().find(|s| s.option_id == OPT_DEAL_STAGE_LEAD).unwrap();
        assert_eq!(lead.entered, 2, "both deals were in Lead");
        assert_eq!(lead.advanced, 1);
        assert!((lead.conversion_rate - 0.5).abs() < f64::EPSILON);
        let qualified = report.steps.iter().find(|s| s.option_id == OPT_DEAL_STAGE_QUALIFIED).unwrap();
    ...
    async fn due_tasks_are_claimed_exactly_once() {
        let store = store().await;
        let record = make_record(&store, "deal", &[("name", json!("Acme")), ("stage", json!("Lead"))]).await;
        store
            .create_activity(&CreateActivityRequest {
        ...
        let overdue = store
            .list_tasks(&TaskQuery { filter: TaskFilter::Overdue, ..Default::default() }, 10, 0)
            .await
            .unwrap();
    ...
    async fn custom_objects_arrive_usable_and_delete_cleanly() {
        ...
        assert_eq!(object.title_field_id.as_deref(), Some(fields[0].id.as_str()));
        ...
        assert_eq!(
            store.search(&SearchQuery { query: "Widget".into(), ..Default::default() }, 10, 0).await.unwrap().total,
            0
        );
    }
    ...
    async fn deleting_a_field_strips_it_from_every_record_and_from_search() {
        ...
        .await;
        assert_eq!(
            store.search(&SearchQuery { query: "ZX9000".into(), ..Default::default() }, 10, 0).await.unwrap().total,
            1
        );
    ...
    async fn schema_boot_call_returns_everything_the_panel_needs() {
        ...
        let deal = schema.objects.iter().find(|o| o.object.slug == "deal").unwrap();
    ...
    async fn a_replace_update_clears_the_fields_it_omits() {
        ...
        let deal = make_record(
            &store,
            "deal",
            &[("name", json!("Acme")), ("stage", json!("Lead")), ("probability", json!(40))],
        )
        .await;
        let update = store
            .update_record(
