...
pub async fn deal_stage_changed(
    ...
    // A stage move is the one CRM event worth a user's attention — raise a
    // notification (the Inbox row shows the Harbor icon) when the new stage has a
    // label to say.
    events
        .emit_with_notify(
            EVENT_DEAL_STAGE_CHANGED,
            ...
            to_label.map(|label| {
                ryu_app_events::NotifyHint::info(
                    format!("Deal moved to {label}"),
                    record_label(record).map(|name| format!("{name} changed stage")),
                )
            }),
        )
        .await;
...
/// Best-effort label for a record's notification title (the record's denormalized
/// display name, when it has one).
fn record_label(record: &Record) -> Option<String> {
...
pub async fn task_due(events: &EventEmitter, activity: &Activity) {
    events
        .emit_with_notify(
            ...
            // A follow-up coming due is exactly what a user wants pushed at them.
            Some(ryu_app_events::NotifyHint::info(
                format!("Follow-up due: {}", activity.title),
                activity.body.as_deref().map(str::to_owned),
            )),
        )
        .await;
}
